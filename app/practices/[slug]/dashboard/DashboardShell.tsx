  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: 20, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
        ...
        {googleReviewCount > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: D.card2, border: `1px solid ${D.border}`, borderRadius: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 26, height: 26, borderRadius: 6, background: 'rgba(66,133,244,0.12)', border: '1px solid rgba(66,133,244,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: D.soft, fontFamily: 'var(--font-body)', marginBottom: 2 }}>Google Reviews</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: D.text, letterSpacing: '-0.02em' }}>
                    {googleAvgRating?.toFixed(1) ?? '—'}
                  </span>
                  <div style={{ display: 'flex', gap: 1 }}>
                    {[1, 2, 3, 4, 5].map(i => (
                      <Star key={i} size={11} strokeWidth={0} fill={googleAvgRating != null && i <= Math.round(googleAvgRating) ? '#fbbf24' : D.faint} />
                    ))}
                  </div>
                  <span style={{ fontSize: 11, color: D.faint, fontFamily: 'var(--font-body)' }}>{googleReviewCount} reviews</span>
                </div>
              </div>
            </div>
            <span style={{ fontSize: 10, color: D.faint, fontFamily: 'var(--font-body)', fontStyle: 'italic' }}>intelligence only</span>
          </div>
        )}
  ...
  return (
    <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 12, padding: '24px 28px', marginBottom: 16 }}>
      ...
      <p style={{ fontSize: 13, color: D.soft, fontFamily: 'var(--font-body)', lineHeight: 1.6, margin: '0 0 16px' }}>
        Import your Google reviews for intelligence analysis. They are used only in your dashboard — never shown publicly on SmileProof.
      </p>
